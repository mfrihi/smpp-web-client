...
class WebSocketHandler {
  ...
  _registerSocketHandlers() {
    ...
    socket.on('message:split_preview', (data) => {
      ...
      // --- Throughput ---
      socket.on('throughput:start',    (data) => this.handleThroughputStart(socket, data));
      socket.on('throughput:pause',    (data) => this.handleThroughputPause(socket, data));
      socket.on('throughput:resume',   (data) => this.handleThroughputResume(socket, data));
      socket.on('throughput:stop',     (data) => this.handleThroughputStop(socket, data));
      socket.on('throughput:update_rate', (data) => this.handleThroughputUpdateRate(socket, data));
      socket.on('throughput:status',   (data) => this.handleThroughputStatus(socket, data));
  ...
  // ===========================================================================
  // Throughput Manager — Rate-limited send with SMSC error handling
  // ===========================================================================

  /**
   * @param {import('./throughput-manager')} throughputManager
   */
  setThroughputManager(tm) {
    this.throughputManager = tm;
  }

  async handleThroughputStart(socket, data) {
    if (!this.throughputManager) return;
    if (!this.smppManager.isConnected) {
      socket.emit('throughput:error', { errorType: 'Connection Error', errorMessage: 'Not connected to SMSC' });
      return;
    }
    try {
      const jobId = this.throughputManager.createJob({
        destinations: data.destinations,
        message: data.message,
        ratePerSecond: data.ratePerSecond,
        totalCount: data.totalCount,
        overrides: data.overrides || {},
        sendCallback: async (dest, msg, opts) => {
          // Retry internally when SMPP window is full — never expose this
          // to the ThroughputManager (it would exhaust retry budget and
          // show false errors).
          for (var attempt = 0; attempt < 300; attempt++) {
            try {
              ...
              const isWindowFull = String(err.message || '').indexOf('Window full') !== -1;
              if (isWindowFull) {
                // Window is full — wait for a slot to open, then retry
                await new Promise(function(r) { setTimeout(r, 50); });
                continue;
              }
              // Real SMPP error — return to throughput manager
              return { success: false, error: err };
            }
          }
          // Exceeded max spin-wait attempts (300 × 50ms = 15s timeout)
          return { success: false, error: new Error('Window full timeout (15s)') };
        },
      });

      // Forward events to this socket with correct name mapping
      const EVENT_MAP = {
        job_started:  'started',
        progress:     'progress',
        smsc_error:   'error',
      ...
      Object.keys(EVENT_MAP).forEach(ev => {
        const fn = (d) => { if (d.jobId === jobId) socket.emit('throughput:' + EVENT_MAP[ev], d); };
        eventFns[ev] = fn;
        this.throughputManager.on(ev, fn);
      });

      this.throughputManager.startJob(jobId);

      // Clean up on disconnect
      socket.once('disconnect', () => {
        if (this.throughputManager.activeJobs && this.throughputManager.activeJobs.has(jobId)) {
          this.throughputManager.stopJob(jobId);
        }
        Object.keys(eventFns).forEach(ev => this.throughputManager.off(ev, eventFns[ev]));
        socket.emit('throughput:cleanup', { jobId });
      });
    } catch (err) {
      socket.emit('throughput:error', { errorType: 'Job Error', errorMessage: err.message });
    }
  }

  handleThroughputPause(socket, { jobId }) {
    if (this.throughputManager && jobId) this.throughputManager.pauseJob(jobId, 'user');
  }

  handleThroughputResume(socket, { jobId }) {
    if (this.throughputManager && jobId) this.throughputManager.resumeJob(jobId);
  }

  handleThroughputStop(socket, { jobId }) {
    if (this.throughputManager && jobId) this.throughputManager.stopJob(jobId);
  }

  handleThroughputUpdateRate(socket, { jobId, newRate }) {
    if (this.throughputManager) this.throughputManager.updateJobRate(jobId, newRate);
  }

  handleThroughputStatus(socket, { jobId }) {
    if (this.throughputManager) {
      const st = this.throughputManager.getJobStatus(jobId);
      socket.emit('throughput:status_response', st);
    }
  }
}
