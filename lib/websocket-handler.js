...
class WebSocketHandler {
  ...
  async handleThroughputStart(socket, data) {
    ...
    try {
      const jobId = this.throughputManager.createJob({
        ...
        sendCallback: async (dest, msg, opts, retryCount) => {
  ...
  handleThroughputPause(socket, { jobId }) {
    if (this.throughputManager) this.throughputManager.pauseJob(jobId, 'user');
  }

  handleThroughputResume(socket, { jobId }) {
    if (this.throughputManager) this.throughputManager.resumeJob(jobId);
  }

  handleThroughputStop(socket, { jobId }) {
    if (this.throughputManager) this.throughputManager.stopJob(jobId);
  }

  handleThroughputUpdateRate(socket, { jobId, newRate }) {
    if (this.throughputManager) this.throughputManager.updateJobRate(jobId, newRate, false);
  }
