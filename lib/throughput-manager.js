...
class ThroughputManager extends EventEmitter {
  ...
  createJob(options) {
    ...
    const targets = destinations.slice(); // copy

    // Per-destination retry counter
    const retryMap = new Map();
    targets.forEach(dest => retryMap.set(dest, 0));
  ...
  updateJobRate(jobId, newRate, autoReduced = false) {
    ...
    // Re-issue interval if running
    if (job.status === 'running' && job.intervalId) {
      clearInterval(job.intervalId);
      const intervalMs = Math.max(10, Math.floor(1000 / job.ratePerSecond));
      job.intervalId = setInterval(() => {
        this._processNext(job).catch(() => {});
